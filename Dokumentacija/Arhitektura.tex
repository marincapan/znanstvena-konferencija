\chapter{Arhitektura i dizajn sustava}
		...
		Arhitektura sustava je izvedena u obliku web aplikacije kojoj korisnik pristupa koristeći web preglednik. Web preglednik omogućuje korisniku pregled web-stranica i multimedijskog sadržaja na njoj. Kako postoje razni web preglednici za razne operativne sustave, koji uglavnom dolaze zajedno s nekim ugrađenim web preglednikom, ovaj pristup omogučava laki i široki pristup aplikaciji bez potrebe za preuzimanjem dodatne programske podrške od strane korisnika kao i dodatnog razvoja od strane programera.
	\par Za razvoj aplikacije koristimo programski jezik Python, HTML, JavaScript i CSS. Za razvoj koristimo razvojni okvir Django koji smo odabrali radi njegove jednostavnosti, modularnosti kao i upoznatosti tima s razvojem u toj okolini.
Korisnik vrši interakcije sa aplikacijom preko django predlozaka. ("templates") Predložci sadrže strukturo potrebnu za dinamičko generiranje HTML stranice i prikaz nje korisniku.
	...
	\begin{figure}[H]
						...
						\centering
						\caption{Dijagram naćina rada djanga}
						\label{fig:dijagramRadaDjanga}
					\end{figure}

		Django predlošci su jedan od sastavnih dijelova radnog okvira Django. Django funcionora na principu rada MVT koji je strukturno vrlo sličan modelu MVC.\newline MVT model se sastoji od:
\begin{itemize}
		\item 	\textbf{Model} - objekti u pythonu koji definiraju strukturu podataka aplikacije i omogučavaju dodavanje, brisanje, mijenjanje i dohvačanje podataka iz baze podataka.
		...
		\item 	\textbf{Template (predložak)} - definira strukturu prikaza podataka neke datoteke (na primjer HTML). Pogledi mogu dinamički generirati HTML stranice uz pomoć predložaka i popuniti ih s podatcima dohvačenih preko modela. 
